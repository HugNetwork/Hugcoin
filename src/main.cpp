...
#if defined(NDEBUG)
# error "Hugcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x46a4e373ff0e4d976265820a38102e9dc841e4180acc198cd915562cae34125f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hugcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hugcoin Signed Message:\n";
...
{
    // Hugcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1 million blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1000000); // Hugcoin: 1 million blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Hugcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Hugcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hugcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hugcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2036c4c40cefdcda2283911db66e57dc36c61da6ae4b033253886c58d0425435");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Star 29/Dec/2018 U.S goverment shutdown heads into second weekend after little progress";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3000000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0405140d7c0636d75871bbf465f1e67c6e9d0286a7789c34cd292fac5c22b8ddbdc3c48cf7859198607d00040f0cf541cb6fd928d6e70cf6323d5a95453d7afc4c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1546130740;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084525780;

        if (fTestNet)
        {
            block.nTime    = 1546130771;
            block.nNonce   = 385577096;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb5e993f0d271a0b217a62f846e423e6a2f225208248d1ce9eef3c23f4e221415"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc2, 0xb4, 0xd8 }; // Hugcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HugcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HugcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HugcoinMiner(CWallet *pwallet)
{
    printf("HugcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hugcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HugcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HugcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HugcoinMiner, pwallet));
}
