...
static void WalCheckpoint(sqlite3* db_) {
  ...
  if (FLAGS_WAL_enabled) {
    sqlite3_wal_checkpoint_v2(db_, NULL, SQHUG_CHECKPOINT_FULL, NULL, NULL);
  }
}
...
class Benchmark {
 private:
  ...
  void PrintEnvironment() {
    fprintf(stderr, "SQHUG:     version %s\n", SQHUG_VERSION);
 ...
 public:
  ...
  void Write(bool write_sync, Order order, DBState state,
    ...
    for (int i = 0; i < num_entries; i += entries_per_batch) {
      ...
      for (int j = 0; j < entries_per_batch; j++) {
        ...
        status = sqlite3_bind_blob(replace_stmt, 1, key, 16, SQHUG_STATIC);
        ErrorCheck(status);
        status = sqlite3_bind_blob(replace_stmt, 2, value,
                                   value_size, SQHUG_STATIC);
        ErrorCheck(status);
        ...
        // Reset SQHUG statement for another use
  ...
  void Read(Order order, int entries_per_batch) {
    ...
    for (int i = 0; i < reads_; i += entries_per_batch) {
      ...
      for (int j = 0; j < entries_per_batch; j++) {
        ...
        status = sqlite3_bind_blob(read_stmt, 1, key, 16, SQHUG_STATIC);
        ErrorCheck(status);

        // Execute read statement
        while ((status = sqlite3_step(read_stmt)) == SQHUG_ROW) {}
        StepErrorCheck(status);
  ...
  void ReadSequential() {
    ...
    ErrorCheck(status);
    for (int i = 0; i < reads_ && SQHUG_ROW == sqlite3_step(pStmt); i++) {
